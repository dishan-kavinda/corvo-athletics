...
- [2026-05-16] **User rejected iterative polish in favor of full ground-up redesign.** When a user says "redesign from scratch", do it completely — new sections, new layout paradigm, new visual hierarchy. Don't just restyle existing sections.
- [2026-05-16] **"Phantom Blade" redesign (v2): crimson `#D81829` is now primary accent, teal `#00BDAC` secondary, gold retained as tertiary.** Violet `#7B5FFF` completely removed. New dark palette: `#07090F` page-bg, `#0C0F1B` surface, `#131825` elevated, `#1B2038` border. New font: Rajdhani (`--font-rajdhani`) added alongside Anton for subheadings/labels. Button `primary` = crimson bg (was white). Footer always dark (hardcoded hex, not CSS vars). Header is client component for scroll-based transparency. ProductCard uses state hover + crimson left border (no TiltCard). MarqueeStrip accepts `light` prop for white text on colored backgrounds. All 9 homepage sections completely different structure from v1. RavenMark SVG (rings+spokes+diagonal blade) used as inline background element. Final CTA section uses full crimson background — only page section with colored bg.
- [2026-05-16] **Raven redesign: violet (`#7B5FFF`) is now primary brand accent, gold is secondary.** [SUPERSEDED by Phantom Blade above] "EDGE." and "POTENTIAL." shimmers use violet→gold→violet gradient. Stats numbers use violet→gold gradient. Marquee alternates violet/gold/muted. Dark mode: `#07080F` page-bg, `#0D0F1A` surface, `#141828` elevated, `#1C2035` border.
...
- [2026-05-16] **"Corvo" means Raven — this identity must be reflected in the design.** Raven aesthetic = dark, iridescent, mysterious + athletic/premium. The correct accent color for raven iridescence is violet `#7B5FFF` (raven feather sheen) + gold `#C9A961` retained as secondary premium signal. Dark mode background updated to cooler midnight blue-black `#07080F` (ravens appear blue-black in light). Abstract feather SVG lines (radiating curves) are the right luxury-brand approach — suggestion over illustration.
- [2026-05-16] **Mobile 3D component was invisible because `opacity-25` (6%) was applied to the particle canvas.** Fix: remove opacity-25, place raven eye orb at `opacity: 0.42` top-right as mobile background element + particles at `opacity: 0.55`. The orb must be present (not just particles) for the raven atmosphere to read on mobile.
- [2026-05-16] **Full-bleed 3D canvas with `var(--hero-gradient)` overlay is the reliable cross-device approach.** Placing the 3D canvas as `absolute inset-0` (full viewport), then applying a directional gradient that fades left-to-right (dark left 40%, transparent right 80%), ensures text is readable on both mobile and desktop without separate mobile/desktop conditional rendering. No `md:hidden` tricks needed.
- [2026-05-16] **Windows build workers can crash with code 3221226505 (STATUS_STACK_BUFFER_OVERRUN) spuriously.** This is a transient OS-level crash in pnpm multi-worker mode, not a code bug. Second build attempt always succeeds. Do not investigate code when this code appears — just rebuild.
- [2026-05-20] **Luxury font: Bodoni Moda (not Playfair Display).** User chose Bodoni Moda for the luxury aesthetic — high-contrast Didone, editorial/fashion-house feel. CSS var `--font-cormorant` kept as the variable name so all luxury components pick it up automatically. Playfair Display felt too generic.
- [2026-05-20] **SplitChooser copy: structural parity between both sides.** User wants matching headline structure — same word count, same layout. Luxury: "GRACE / UNDER / PRESSURE." (Bodoni Moda). Savage: "HUNT / WITHOUT / MERCY." (Anton). One word per line, accent colour on final word.
